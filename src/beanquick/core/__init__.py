"""This module provides the data required by Beanquick's reports."""
...
from beanquick.beans.account import get_entry_accounts
from beanquick.beans.funcs import get_position
from beanquick.beans.funcs import hash_entry
from beanquick.beans.helpers import slice_entry_dates
from beanquick.beans.load import load_uncached
from beanquick.beans.prices import FavaPriceMap
from beanquick.beans.str import to_string
from beanquick.core.accounts import AccountDict
from beanquick.core.attributes import AttributesModule
from beanquick.core.budgets import BudgetModule
from beanquick.core.charts import ChartModule
from beanquick.core.commodities import CommoditiesModule
from beanquick.core.conversion import cost_or_value
# from beanquick.core.extensions import ExtensionModule
from beanquick.core.beanquick_options import parse_options
from beanquick.core.file import _incomplete_sortkey
from beanquick.core.file import FileModule
from beanquick.core.file import get_entry_slice
from beanquick.core.filters import AccountFilter
from beanquick.core.filters import AdvancedFilter
from beanquick.core.filters import TimeFilter
from beanquick.core.group_entries import group_entries_by_type
# from beanquick.core.ingest import IngestModule
from beanquick.core.inventory import CounterInventory
from beanquick.core.misc import FavaMisc
from beanquick.core.number import DecimalFormatModule
from beanquick.core.query_shell import QueryShell
from beanquick.core.tree import Tree
from beanquick.core.watcher import Watcher
from beanquick.core.watcher import WatchfilesWatcher
from beanquick.helpers import BeanquickError
from beanquick.util import listify
from beanquick.util.date import dateranges
...
class StatementNotFoundError(BeanquickError):
    """Statement not found."""
...
class StatementMetadataInvalidError(BeanquickError):
...
class FilteredLedger:
    ...
    def __init__(
        ...
        *,
        ...
        if time:
            time_filter = TimeFilter(ledger.options, ledger.beanquick_options, time)
    ...
    def root_tree_closed(self) -> Tree:
        ...
        tree.cap(self.ledger.options, self.ledger.beanquick_options.unrealized)
        return tree
...
class BeanquickLedger:
    ...
    beanquick_options_errors: Sequence[BeancountError]
    ...
    async def async_load_file(self) -> bool:
        """Asynchronously load the main file and all included files."""
        async with self._loading_lock:
            try:
                await asyncio.to_thread(self.load_file) # it'll crash sometimes if open another file
                # self.load_file()  # Call the synchronous load_file directly
                self.is_loaded = True
                return True
            except Exception as e:
                self.is_loaded = False
                logger.error(f"Async load failed for {self.beancount_file_path}: {e}", exc_info=True)
                return False
    ...
    def load_file(self) -> None:
        ...
        self.beanquick_options, self.beanquick_options_errors = parse_options(
    ...
    def changed(self) -> bool:
        ...
        if self._is_encrypted:
            return False
        if not self._watcher_active: # If watcher isn't active, it can't detect changes
            logger.debug(f"Watcher for {self.beancount_file_path} is not active, cannot check for changes.")
            # Optionally, try to reactivate it if it should be active
            # self.ensure_watcher_running()
            # if not self._watcher_active: # Check again
            # return False
            return False

        # Ensure watcher has been initialized by update() at least once
        if not hasattr(self.watcher, '_watchers') or self.watcher._watchers is None:
            if not self._is_encrypted:
                 logger.warning(f"Watcher for {self.beancount_file_path} not fully initialized. Attempting to update.")
                 # This implies a state where _watcher_active might be true but watcher.update was never called
                 # or threads were never created. load_file should ideally prevent this.
                 try:
                     files_to_watch, folders_to_watch = self.paths_to_watch()
                     self.watcher.update(files_to_watch, folders_to_watch)
                     self._watcher_active = True # Re-affirm
                 except Exception as e:
                     logger.error(f"Error re-initializing watcher in changed() for {self.beancount_file_path}: {e}")
                     self._watcher_active = False # Mark inactive on error
                     return False # Can't check if watcher init fails
            else: # Encrypted, shouldn't be here if _watcher_active is true
                self._watcher_active = False
                return False


        changed_detected = self.watcher.check()
        if changed_detected:
            logger.info(f"Change detected by watcher for {self.beancount_file_path}. Reloading.")
            self.load_file() # This will also call watcher.update()
    ...
    def _deactivate_watcher_if_active(self) -> None:
        """Helper to stop watcher threads if they are running and mark inactive."""
        if self._watcher_active:
            if hasattr(self.watcher, '_watchers') and self.watcher._watchers is not None:
                logger.info(f"Deactivating watcher for {self.beancount_file_path}")
                try:
                    self.watcher.__exit__(None, None, None) # Stops threads
                    logger.debug(f"Watcher __exit__ called for {self.beancount_file_path}")
                except Exception as e:
                    logger.error(f"Error during watcher.__exit__ for {self.beancount_file_path}: {e}", exc_info=True)
            else:
                logger.debug(f"Watcher for {self.beancount_file_path} marked active but no threads found to stop.")
            self._watcher_active = False
        else:
            logger.debug(f"Watcher for {self.beancount_file_path} already inactive or no need to deactivate.")

    def ensure_watcher_running(self) -> None:
        """
        Ensures the watcher is running if the ledger is loaded, not encrypted,
        and the watcher is not already marked as active.
        This is typically called when a loaded ledger becomes the active view.
        """
        if not self.is_loaded:
            logger.debug(f"Watcher not started for {self.beancount_file_path}: Ledger not loaded.")
            return
        if self._is_encrypted:
            logger.debug(f"Watcher not started for {self.beancount_file_path}: File is encrypted.")
            self._deactivate_watcher_if_active() # Ensure it's off if it somehow was on
            return

        if not self._watcher_active:
            logger.info(f"Watcher for {self.beancount_file_path} is not active. Attempting to start/restart.")
            try:
                # self.options should be populated if self.is_loaded is True from a successful load_file
                if not self.options:
                    logger.warning(f"Cannot start watcher for {self.beancount_file_path}: self.options not available."
                                       " A full load might be needed.")
                    return

                self.watcher.update(*self.paths_to_watch())
                self._watcher_active = True
                logger.info(f"Watcher successfully (re)activated for {self.beancount_file_path}")
            except Exception as e:
                logger.error(f"Failed to (re)activate watcher for {self.beancount_file_path}: {e}", exc_info=True)
                self._watcher_active = False # Ensure it's marked inactive on error
        else:
            logger.debug(f"Watcher for {self.beancount_file_path} is already active.")

    
    def stop_watcher(self) -> None:
        """Public method to explicitly stop the watcher."""
        logger.info(f"External request to stop watcher for {self.beancount_file_path}")
        self._deactivate_watcher_if_active()
