...
Options for Beanquick can be specified through Custom entries in the Beancount file.
...
from beanquick.util import get_translations
from beanquick.util.date import END_OF_YEAR
from beanquick.util.date import parse_fye_string
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from beanquick.beans.abc import Custom
    from beanquick.util.date import FiscalYearEnd


class OptionError(BeancountError):
    """An error for one the Beanquick options."""
...
class UnsupportedLanguageOptionError(ValueError):  # noqa: D101
    def __init__(self, value: str) -> None:
        super().__init__(f"Beanquick has no translations for: '{value}'.")
...
@dataclass
class FavaOptions:
    """Options for Beanquick that can be set in the Beancount file."""
...
def parse_option_custom_entry(entry: Custom, options: FavaOptions) -> None:
    """Parse a single custom beanquick-option entry and set option accordingly."""
...
def parse_options(
    ...
    """Parse custom entries for Beanquick options.

    The format for option entries is the following::

        2016-04-01 custom "beanquick-option" "[name]" "[value]"
    ...
    for entry in (e for e in custom_entries if e.type == "beanquick-option"):
        try:
            if not entry.values:
                raise MissingOptionError
            parse_option_custom_entry(entry, options)
        except (IndexError, TypeError, ValueError) as err:
            msg = f"Failed to parse beanquick-option entry: {err!s}"
