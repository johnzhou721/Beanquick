...
from beanquick.beans.flags import FLAG_CONVERSIONS
from beanquick.beans.flags import FLAG_MERGING
from beanquick.beans.flags import FLAG_PADDING
from beanquick.beans.flags import FLAG_RETURNS
from beanquick.beans.flags import FLAG_SUMMARIZE
from beanquick.beans.flags import FLAG_TRANSFER
...
from beanquick.util import next_key
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from beanquick.core.beanquick_options import InsertEntryOption
...
class NonSourceFileError(BeanquickError):
...
class ExternallyChangedError(BeanquickError):
...
class InvalidUnicodeError(BeanquickError):
...
class FileModule(FavaModule):
    ...
    def insert_metadata(
        ...
        with self._lock:
            ...
            indent = self.ledger.beanquick_options.indent
    ...
    def save_entry_slice(
        ...
        Raises:
            BeanquickError: If the entry is not found or the file changed.
        """
        with self._lock:
            entry = self.ledger.get_entry(entry_hash)
    ...
    def insert_entries(self, entries: Sequence[Directive]) -> None:
        ...
        with self._lock:
            # self.ledger.changed()
            beanquick_options = self.ledger.beanquick_options
            for entry in sorted(entries, key=_incomplete_sortkey):
                path, updated_insert_options = insert_entry(
                    entry,
                    (
                        self.ledger.beanquick_options.default_file
                        or self.ledger.beancount_file_path
                    ),
                    insert_options=beanquick_options.insert_entry,
                    currency_column=beanquick_options.currency_column,
                    indent=beanquick_options.indent,
                )
                self.ledger.watcher.notify(path)
                self.ledger.beanquick_options.insert_entry = updated_insert_options
                # self.ledger.extensions.after_insert_entry(entry)

    def insert_entries_to_file(self, entries: Sequence[Directive], file_path: str) -> None:
        """Insert entries to a specific file.

        Args:
            entries: A list of entries.
            file_path: The file path to insert entries into.
        """
        with self._lock:
            # self.ledger.changed()
            beanquick_options = self.ledger.beanquick_options
